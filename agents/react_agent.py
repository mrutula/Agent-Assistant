...
while i <= max_iterations:
    ...
    response = client.responses.create(
        model="gpt-4.1-mini", instructions=s.react_prompt, input=input_list
    )
    try:
    ...
    if "action" in output_dict:
        ...
        logger.info(
            f"Action: Use the function: {func_name}.Reason: {func_reason}, using these arguments {func_arg}"
        )
                    ...
                    "content": json.dumps(
                        {
                            ...
                            "message": "Invalid function name. Please choose a tool from the provided list. If no tool can be chosen and you don't know the answer please reply 'Dont know the answer' ",
                        }
                    ),
                }
            )
